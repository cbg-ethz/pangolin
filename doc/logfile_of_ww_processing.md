...
11 August 2026:
- SPSP upload:
  - stoped as it did not find the respecive files:

```
====================
Start new UPLOADER run
====================
Checking the upload quotas:
----
Daily sample number: 0/1000000000000
Daily size: 0/500000 MB
----
asked to upload 100 new samples, consisting of about 50000 MB
starting UPLOADER job
/app/pangolin_src vs /app/workdir
belfry.sh upload
uploading 100 samples from the list of samples to upload
Preparing the list of files to upload for this batch
Downloading from Euler the necessary cram files
CHECK one
receiving incremental file list
rsync: [sender] link_stat "A1_05_2025_02_10/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "A2_15_2025_02_11/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "A3_17_2025_02_18/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "B1_05_2025_02_14/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "B2_15_2025_02_14/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "B3_17_2025_02_22/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [sender] link_stat "C1_10_2025_02_10/20250307_2418539124/uploads/dehuman.cram" (in working) failed: No such file or directory (2)
rsync: [Receiver] ERROR: cannot stat destination "/app/raw_data/working/samples/": Permission denied (13)
rsync error: errors selecting input/output files, dirs (code 3) at main.c(768) [Receiver=3.2.3]
Cleaning the temporary folders generated by SendCrypt

```

```
- Die files werden in /app/raw_data/working/samples gesucht. Dort fehlen die samples komplett.
- auf Euler wurden die files aber erstellt und sind vorhanden
- darum nehme ich an das es beim sync auf die wisedb probleme gibt
- 
- im ordner /app/raw_data/working/samples wurde am 26 July 2025 das letztemal files geschrieben.
- 
- die funktion `pullsamples_for_db ` uploaded samples in diese destination
- die funktion ist in der automation auskommentiert, da sie wohl ein bug hat: # BUG: will generate non-globed pattern if months are missing
```

pullsamples_for_db: on euler and in the running sars_cov docker
- ersetze globstar mit nullglob --> so it only uses fiels that exist
- uncomment the function in carillon.sh (note form later: commented the original parts where the function was called again because they made no sense, added the function to the uploader section)

added part to the uploader section that pulls data from euler to wisedb:
...
```
- adapted file path "sampleset" to the new path after restructuring: ${clusterdir_old}/${clusterdir}/${sampleset}
- "ERROR: The remote path must start with a module name " sampleset" inthe above context is the mudule name on euler not the path --> checked and reverted the path change before

- rsync: [sender] link_stat "samples.202608*.tsv" (in vpipe_input) failed: No such file or directory (2)
- added --ignore-missing-args \ to rsync function such that there is not yet a sample set file from this month it is ignored instread of erroring out

Next error: in function "callpullrsync_fordb"
```
pulling recent:
pulling: app/workdir/tmp/belfrysheets/samples.20260710_2540504211.tsv app/workdir/tmp/belfrysheets/samples.20260724_2539415129.tsv
samples:
rsync: [sender] change_dir "samples" (in working) failed: No such file or directory (2)

```
- corrected pat from arglist=( "${@/#/belfry@euler.ethz.ch::${working}/samples/}" ) to arglist=( "${@/#/belfry@euler.ethz.ch::${working}/}" )

- added --year flag to function pullsamples_for_db, to get all 2025 samples for the spsp upload

once this works, 
- acitvate that it should sync all files by removind the --recent flag in the running docker (function should sync all if no flag is given)
- document
- why are the sampe data for the spsp uploader not uploaded to the destination where the rest of the sample data is stored?

20 July 2026:

- created technical user account pangolin on fgcz and added it to project
- changed the password files on euler and wisedb

17 July 2026:

- donotsubmit_uploader=1 in the sars cov container to test the spsp uploader
- did not work revertet to 0

```Bash
Sample not found in timeline file.
G2_10_2025_03_06 20250321_2429695737
Generating metadata line for sample G2_10_2025_03_06
CHECK wisedb_token /run/secrets/wisedb_token
Sample not found in timeline file.
G3_16_2025_03_06 20250321_2429695737
Generating metadata line for sample G3_16_2025_03_06
CHECK wisedb_token /run/secrets/wisedb_token
Sample not found in timeline file.
Cleaning the temporary folders generated by SendCrypt
/app/pangolin_src vs /app/workdir
belfry.sh clean_sendcrypt_temp
Clearning the sendcrypt temporary directories in /app/workdir/uploader/temp
\e[33;1mBackup of UPLOADER results on bs-bewi08 DISABLED\e[0m

```

14 July 2026:

- no ouput of the new batch even though new sequencing is available at bfabric
- observed unusual logs in fgcz sync automation:
  
```Bash
file has vanished: "/p23224/o40780_Aviti_260109_AV147/A5_16_2025_12_19_R1.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/A5_16_2025_12_19_R2.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/A6_25_2025_12_23_R1.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/A6_25_2025_12_23_R2.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/B4_05_2025_12_21_R1.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/B4_05_2025_12_21_R2.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/B5_16_2025_12_22_R1.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/B5_16_2025_12_22_R2.fastq.gz" (in bfabric-downloads)
file has vanished: "/p23224/o40780_Aviti_260109_AV147/B6_25_2025_12_26_R1.fastq.gz" (in bfabric-downloads)
rsync error: received SIGINT, SIGTERM, or SIGHUP (code 20) at io.c(515) [receiver=3.2.5]
rsync error: unexplained error (code 255) at rsync.c(713) [generator=3.2.5]
rsync: [generator] write error: Broken pipe (32)
FAILED
Tue, 14 Jul 2026 07:07:28 +0000

```

- in batman.sh fgcz_scyn changed the sync date from 6 month to 2 month, to see if this error also concernes the latest batches
- this is the output from the backup, so nothing to worry about
- --> might be due to the server patching? check later if persisting

- however the mirror errors out:
```Bash
lftp -c set cmd:move-background false; set net:timeout 30; set net:max-retries 10; set net:reconnect-interval-base 8; set xfer:timeout 300; connect sftp://carrara:<PASSWORD>@fgcz-gstore.uzh.ch:666; cd /projects; mirror --only-newer --continue --no-perms --parallel=8 --loop  --directory=p23224 -O /cluster/project/pangolin/data/fgcz_raw  --newer-than='20260514'  --exclude-rx-from='/cluster/project/pangolin/processes/status/sync/fgcz.exclude.lst'
mirror: Access failed: Permission denied
```
- i assume the cause is the login with carrara Account (saw the same error already on monday 13th july)
- changed the pw file `fgcz-gstore.uzh.ch` on wisedb and euler (.ssh)  (currently it uses a personal account)
  - asked fgcz for support to setup a technical user account! [done on 20 july 26: set it up and chance credentials on euler and wisedb]


13 July 2026:

- (spsp uploader)changed parameter : donotsubmit_uploader=1 so it only updated file list but does not run the spsp uploader (because the uploader blocks the start of the new batch analysis )
- reverted this flag after changing a function in create metadata line :
...
def load_timeline(timelinefile, samplename):
    with open(timelinefile, "r") as file:
        ...
        for line in timeline:
            if (samplename in line):
                return line
    sys.exit("Sample not found in timeline file.")

```

9 July 2026:

- cleanedup covid slurm logs in folder: `bs-pangolin@eu-login-20:/cluster/project/pangolin/processes/sars_cov_2/pangolin/working`
  - created folder "slurm_logs" and moved all slurm-* files inside
  - same for folder `/cluster/project/pangolin/processes/sars_cov_2/working`
  - changed path in vpipe.sbatch file to write the logs in dedicated folder: `/cluster/project/pangolin/processes/sars_cov_2/pangolin/working/vpipe_aviti.sbatch`
- continued spsp upload debugging
...
- observation: Mail On 1 Sep 2025, at 10:27: "we noted that the ARTIC version changed from v5.3.2 to v5.4.2"
  - in the mail they stated:" we’ve already used the 5.4.2 version for the following order placed on 31/3/25 (o38166), and all subsequent orders are using this version."
  - realted git issue in dev board: https://github.com/orgs/cbg-ethz/projects/10/views/1?pane=issue&itemId=131565024&issue=cbg-ethz%7CDevelopment_privat%7C91
  - it was decided that there is no need to rerun the batches with vpipe
  - pending: correct the metadata on spsp
- spsp upload stoped in august 2025 (during the restructuring)

16 June 2026:

- added notes on how to add lollipop to automation run: `/cluster/project/pangolin/research/NEXUS/20260616_adding_vpipe_lollipop/README.md`
- 

15 June 2026:

- Processing of bathc 20260612_2530556724
- Samples with very low coverage / dropouts: A3_17_2026_05_28,C3_25_2026_05_28,G1_25_2026_05_20,F3_15_2026_05_31,F1_17_2026_05_22

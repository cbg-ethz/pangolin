...
case "$1" in
    syncopenbis)
        ...
        syncoutput="$(/usr/bin/time ${scriptdir}/sync_sftp.sh -c ${scriptdir}/config/gfb.conf "${param[@]}"|tee /dev/stderr)"
    ...
    sortsamples)
        ...
        fail=0
        if  (( ${lab[gfb]} )); then
            ${scriptdir}/sort_samples_pybis.py -c ${scriptdir}/config/gfb.conf --protocols=${working}/${protocolyaml} --assume-same-protocol ${force} ${summary} ${recent} && bash ${basedir}/${sampleset}/movedatafiles.sh || fail=1
        else
            echo "Skipping gfb"
        fi
        if  (( ${lab[fgcz]} )); then
#           # TODO config file
	...
	rsync -e "${remote_batman} rsync" \
        ...
        timeout ${timeoutforeground} --signal=INT --kill-after=5 $((rsynctimeout+contimeout+5)) \
            rsync   --timeout=${iotimeout}  \
            -e "ssh -i ${privkey} ${cluster_user}@${cluster}  -oConnectTimeout=${contimeout}"   \
	...
	# todo: secrets
        rsync   \
            -e "ssh -i ${privkey} ${cluster_user}@${cluster}" \
	    --itemize-changes \
	    --compress \
	    --recursive \
	    --links \

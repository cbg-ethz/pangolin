...
case "$1" in
	...
	clean_preproc_flu)
		type="flu"
		virusbase=("influenza/IA_H1" "influenza/IA_H3" "influenza/IA_MP" "influenza/IA_N1" "influenza/IA_N2")
		parent="working/results"
		togarbage=("preprocessed_data/*fastq.gz")
		if [[ $doit == true ]]; then
                        garbage_logfile=${garbage_logs}/$(date '+%Y%m%d')_${type}_preproc.log
...
if [[ "$type" == "fgcz" ]];then
	for prj in "${projlist[@]}"; do
		garbagedir="${clusterdir_old}/data/garbage/${type}/${prj}"
		readarray -t dirs < <(
	...
	garbagedir="${clusterdir_old}/data/garbage/lollipop"
	...
	while IFS=$'\t' read -r sample batch _rest; do
		for item in "${togarbage[@]}"; do
			...
			garbagedir="${clusterdir_old}/data/garbage/${type}/${base}/${sample}/${batch}/${item%/*}"
	...
	for subtype in "${virusbase[@]}"; do
		gawk -v d="${custom_date}" '$2 < d' \
			"${clusterdir_old}/processes/${subtype}/working/samples.tsv" > "${clusterdir_old}/processes/${subtype}/working/samples.tsv.toclean"
		while IFS=$'\t' read -r sample batch _rest; do
			for item in "${togarbage[@]}"; do
				tomove="${clusterdir_old}/processes/${subtype}/${parent}/${sample}/${batch}/${item}"
				subtype_garbagedir=${subtype#*/}
				garbagedir="${clusterdir_old}/data/garbage/${type}/${subtype_garbagedir}/${parent}/${sample}/${batch}/${item%/*}"
		...
		done < "${clusterdir_old}/processes/${subtype}/working/samples.tsv.toclean"
	done
fi
